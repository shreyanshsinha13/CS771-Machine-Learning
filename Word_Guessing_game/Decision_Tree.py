...
def my_fit( words, verbose = False ):
	...
	percentage = 0.5
	n = int( percentage * len( words ) / 100)
	dt.fit( n, words, verbose )
	return dt
...
class Tree:
	...
	def fit( self, n, words, verbose = False ):
		self.words = words
		...
		self.root.fit( n, all_words = self.words, my_words_idx = np.arange( len( self.words ) ), min_leaf_size = self.min_leaf_size, max_depth = self.max_depth, verbose = verbose )
...
class Node:
	...
	def process_leaf( self, my_words_idx ):
		# idx = np.random.choice( my_words_idx )
		# return idx
	...
	def entropyfn( self, nt, split_dict ):
		entropy = 0
		for mask, idx in split_dict.items():
			# if mask == his:
			# 	continue
			nti = len( idx )
			entropy += ( nti / nt * np.log2( nti / nt ) )

		return -entropy
	
	# Dummy node splitting action -- use a random word as query
	# Note that any word in the dictionary can be the query
	def process_node( self, n, all_words, my_words_idx, verbose ):
		...
		if self.depth == 0:
			...
			idxs = my_words_idx
			N = n
			if N < len ( my_words_idx ):
				idxs = np.random.choice( my_words_idx, N, replace = False )
			for index in idxs:
				temp_query = all_words[ index ]
				temp_split_dict = {}
				for idx in my_words_idx:
					if idx == index:
						continue
					mask = self.reveal( all_words[ idx ], temp_query )
					if mask not in temp_split_dict:
						temp_split_dict[ mask ] = []
					
					temp_split_dict[ mask ].append( idx )

				temp_entropy = self.entropyfn( len( my_words_idx ), temp_split_dict )
				if temp_entropy > entropy:
					entropy = temp_entropy
					query_idx = index
					query = temp_query
					split_dict = temp_split_dict
	...
	def fit( self, n, all_words, my_words_idx, min_leaf_size, max_depth, fmt_str = "    ", verbose = False ):
		...
		if len( my_words_idx ) <= min_leaf_size or self.depth >= max_depth:
			...
			self.is_leaf = False
			( self.query_idx, split_dict ) = self.process_node( n, all_words, self.my_words_idx, verbose )
			...
			for ( i, ( response, split ) ) in enumerate( split_dict.items() ):
				...
				self.children[ response ].fit( n, all_words, split, min_leaf_size, max_depth, fmt_str, verbose )